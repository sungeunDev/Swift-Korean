...
Swift는 어떤 값의 부재를 다룰 수 있는 d 타입도 제공 합니다. 이 선택형은 "값이 존재하고, 그 값은 x입니다." 혹은 "값이 존재 하지 않습니다."라고 할 수 있습니다. 선택형은 Objective-C의 포인터에서 `nil`을 사용하는 것과 비슷합니다. 하지만 클래스 뿐만 아니라 어떤 타입에도 사용할 수 있습니다. 선택형은 Objective-C의 `nil` 포인터보다 훨씬 안전하고 쓰임새 있습니다. 또 Swift의 강력한 기능들중 핵심적인 기능입니다.
...
`print(_:separator:terminator:)`은 하나 이상의 값을 출력해주는 전역 함수입니다. Xcode에서 작업을 하고 있다면 `print(_:separator:terminator:)`을 사용하면 Xcode의 "console"창에 결과가 나옵니다. `separator`와 `terminator` 매개 변수에는 기본값이 있으므로 함수를 호출할 때 매개변수를 생략할 수 있습니다. 기본적으로 이 함수는 줄 바꿈을 추가하여 한 줄 출력을 종료합니다. 줄바꿈 없이 값을 출력하려면 빈 문자열을 종결자로 전달하세요. 예를 들어 `print(someValue, terminator: "")`. 기본 값이 있는 매개 변수에 대한 자세한 내용은 `Default Parameter Values`를 참고하세요.
